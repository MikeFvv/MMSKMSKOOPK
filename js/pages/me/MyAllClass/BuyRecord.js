...
export default class BuyRecord extends Component {
            ...
            <CustomNavBar
                ...
                centerView={
                    (
                        ...
                        <TouchableOpacity
                            activeOpacity={0.7} onPress={() =>navigation.state.params.navTitlePress()}>
                            <View style={{
                                ...
                                <View style={{
                                    ...
                                    <Text
                                        allowFontScaling={false} style={{ fontSize: Adaption.Font(17), color: 'white',marginLeft:5}}>{navigation.state.params.navcLTitle != '' ? navigation.state.params.navcLTitle :''}</Text>
                                <Image style={{ width:15, height: 15,marginTop:3,marginLeft:15}} source={require('../MyAllClass/BettingRecord/img/ic_sanjiao.png')}></Image>
                                </View>
                            </View>
                        ...
                        navigation.state.params ? (
                            <TouchableOpacity activeOpacity={1} style={{
                              ...
                              <CusBaseText style={{ fontSize: 14, color: 'white', textAlign: 'center' }}>
                                         {navigation.state.params.choiceData != '' ? navigation.state.params.choiceData :''}
                              </CusBaseText>
                              <Image style={{width: 15 * KAdaptionWidth, height: 17 * KAdaptionHeight,marginLeft:5}}
                                     source={require("../MyAllClass/BettingRecord/img/ic_xiangxia.png")}
                              />
                            </TouchableOpacity>) : null
                    )
                }
    ...
    renderCell = (item) => { // cell 的视图渲染
        ...
        console.log('提款记录',statuStr);

                
        const { navigate } = this.props.navigation;
        return <TouchableOpacity activeOpacity={1} style={styles.cellStyle} onPress={() => this._onClickCell(item, navigate)} >
            <View style={{ flex: 0.6,}}>
                <CusBaseText allowFontScaling={false} style={{ width: 100, textAlign: 'center', marginTop: 14,}}>{isStatu}</CusBaseText>
                <CusBaseText allowFontScaling={false} style={{ marginLeft: 5, marginTop: 10, color: '#666666', fontSize: Adaption.Font(13, 11),marginTop: 12, }}>单号{item.item.value.order !=''  &&  item.item.value.order != undefined ? item.item.value.order :'无单号'}</CusBaseText>
            </View>
            <View style={{ flex: 0.4, flexDirection: 'row' }}>
                <View style={{ flex: 0.88 }}>
                    <CusBaseText allowFontScaling={false} style={{ marginRight: 1, color: 'red', textAlign: 'right', marginTop: 12, }}>{item.item.value.price !=''  && item.item.value.price != undefined  ? item.item.value.price :'0.00'}元</CusBaseText>
                    <CusBaseText allowFontScaling={false} style={{ marginRight: 1, color: '#666666', textAlign: 'right', marginTop: 12, fontSize: Adaption.Font(13, 11), }}>{item.item.value.time}</CusBaseText>
                </View>
                <View style={{ flex: 0.12, alignItems: 'center', justifyContent: 'center' }}>
                    <Image style={{ width: 15, height: 15, alignItems: 'center' }}
                        source={require("../Me_Ico/ic_Black_arrow.png")}
                    />
                </View>
            </View>
        </TouchableOpacity>
    ...
    listEmptyComponent() {
        if (this.jiaZai == 0) {
            ...
            return (
                <View style={{ width: width, height: height - height / 4, justifyContent: 'center', alignItems: 'center' }}>
                    <Image resizeMode={'stretch'} style={{ width: 60 * KAdaptionWidth, height: 50 * KAdaptionHeight }} source={require('../MyAllClass/BettingRecord/img/ic_noData.png')} />
